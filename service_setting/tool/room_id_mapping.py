...
room_mapping_dict = {}
for floor in setting.keys():
    for item in setting[floor]:
        if item != []:
            room_mapping_dict[floor + '_' + item[0]] = item[1]
        else:
            print('Got empty list.')

ros_package = rospkg.RosPack()
param_path = ros_package.get_path('robot_unique_parameters')
param_path += '/service_setting/room_id_mapping.txt'


with open(param_path, 'w') as f:
    json.dump(room_mapping_dict, f)
